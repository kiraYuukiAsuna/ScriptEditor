...
std::unique_ptr<antlr4::Token> Python3LexerBase::nextToken()
{
    if (_input->LA(1) == EOF && !indents.empty()) {
        ...
        emit(commonToken(antlrpython3::Python3Parser::NEWLINE, "\n"));
        while (!indents.empty()) {
            emit(createDedent());
            indents.pop();
        }
...
std::unique_ptr<antlr4::Token> Python3LexerBase::createDedent() {
    std::unique_ptr<antlr4::CommonToken> dedent = commonToken(antlrpython3::Python3Parser::DEDENT, "");
    return dedent;
}
...
void Python3LexerBase::onNewLine()
{
    ...
    if (opened > 0 || (nextnext != -1 && (next == '\r' || next == '\n' || next == '\f' || next == '#'))) {
        ...
        emit(commonToken(antlrpython3::Python3Lexer::NEWLINE, newLine));
        ...
        if (indent == previous) {
            ...
            indents.push(indent);
            emit(commonToken(antlrpython3::Python3Lexer::INDENT, spaces));
        }
        else {
